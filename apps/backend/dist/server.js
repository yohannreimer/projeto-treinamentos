...
const INSTALLATION_CODES = ['960001010', 'MOD-01'];
...
function getInstallationModule() {
    ...
    limit 1
  `).get(INSTALLATION_CODES[0], INSTALLATION_CODES[1], INSTALLATION_CODES[0], INSTALLATION_CODES[1]);
    return row ?? null;
}
function getInstallationModuleId() {
    return getInstallationModule()?.id ?? null;
}
function getInstallationModuleCode() {
    return getInstallationModule()?.code ?? null;
}
function hasModuleEnabled(companyId, moduleId) {
    const row = db.prepare(`
...
function ensureCompanyDefaultRows(companyId) {
    const moduleRows = db.prepare('select id from module_template').all();
...
function hasModuleCompleted(companyId, moduleId) {
    const row = db.prepare(`
    select 1 as ok
...
function moduleExistsInCohort(cohortId, moduleId) {
    const row = db.prepare(`
    select 1 as ok
    from cohort_module_block
    where cohort_id = ? and module_id = ?
    limit 1
  `).get(cohortId, moduleId);
    return Boolean(row?.ok);
}
...
function normalizeBusinessStart(dateIso) {
...
function addBusinessDays(dateIso, offset) {
...
function cohortBusinessDates(startDate, blocks) {
    const totalDays = blocks.length === 0
        ? 1
        : Math.max(1, ...blocks.map((block) => block.start_day_offset + block.duration_days - 1));
    const dates = [];
...
function renewalAlertWindowDays(renewalCycle) {
    return renewalCycle === 'Anual' ? 30 : 7;
}
function nextRenewalDate(currentExpiresAt, renewalCycle) {
...
function findTechnicianConflict(params) {
  ...
  `);
    for (const cohort of candidateCohorts) {
        const existingBlocks = selectBlocks.all(cohort.id);
...
function getAdminCatalog() {
    const modules = db.prepare('select * from module_template order by code asc').all();
    ...
    return {
        modules: modules.map((module) => ({
            ...
            prerequisites: (() => {
                ...
                if (!installationModule)
                    return [];
                if (module.id === installationModule.id)
                    return [];
                if (explicit.some((item) => item.id === installationModule.id))
                    return explicit;
...
app.get('/dashboard', (_req, res) => {
  ...
  `).get(installationCode, installationCode) : { count: 0 };
...
app.post('/cohorts/check-technician-conflict', (req, res) => {
    ...
    const technician = db.prepare('select id, name from technician where id = ?').get(payload.technician_id);
    if (!technician) {
...
app.post('/cohorts', (req, res) => {
    ...
    const tx = db.transaction(() => {
      ...
      values (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cohortId, payload.code.toUpperCase(), payload.name, payload.start_date, payload.technician_id ?? null, payload.status, payload.capacity_companies, payload.notes ?? null);
        const insertBlock = db.prepare(`
      insert into cohort_module_block (id, cohort_id, module_id, order_in_cohort, start_day_offset, duration_days)
      values (?, ?, ?, ?, ?, ?)
    `);
        payload.blocks.forEach((block) => {
            insertBlock.run(uuid('blk'), cohortId, block.module_id, block.order_in_cohort, block.start_day_offset, block.duration_days);
        });
    });
    try {
        tx();
...
app.patch('/cohorts/:id', (req, res) => {
  ...
  `).all(req.params.id);
    ...
    const tx = db.transaction(() => {
      ...
      `);
            payload.blocks.forEach((block) => {
                insertBlock.run(uuid('blk'), req.params.id, block.module_id, block.order_in_cohort, block.start_day_offset, block.duration_days);
            });
        }
    });
    try {
...
app.delete('/cohorts/:id', (req, res) => {
    const exists = db.prepare('select id from cohort where id = ?').get(req.params.id);
    if (!exists) {
...
app.post('/allocations', (req, res) => {
    ...
    from cohort_module_block
    where cohort_id = ? and module_id = ?
  `).get(payload.cohort_id, payload.module_id);
    if (!block) {
    ...
    const cohort = db.prepare('select capacity_companies from cohort where id = ?').get(payload.cohort_id);
    if (!cohort) {
        return res.status(404).json({ message: 'Turma nao encontrada' });
    }
...
app.post('/cohorts/:id/allocate-company', (req, res) => {
  ...
  `).all(cohortId);
    ...
    const company = db.prepare('select id from company where id = ?').get(payload.company_id);
    if (!company) {
  ...
  `).get(cohortId);
    ...
    where cohort_id = ? and company_id = ? and status <> 'Cancelado'
    limit 1
  `).get(cohortId, payload.company_id);
    if (occupied.count >= cohort.capacity_companies && !alreadyInCohort) {
        return res.status(400).json({ message: 'Capacidade da turma atingida' });
    }
  ...
  `);
    const tx = db.transaction(() => {
        selectedBlocks.forEach((block) => {
            saveAllocation.run(uuid('all'), cohortId, payload.company_id, block.module_id, block.start_day_offset, payload.notes ?? null);
        });
    });
    try {
        tx();
  ...
  `).all(...selectedBlocks.map((block) => block.module_id));
...
app.patch('/allocations/:id/status', (req, res) => {
  ...
  `).get(req.params.id);
    ...
    where id = ?
  `).run(nextStatus, parsed.data.notes ?? null, usedInstallationOverride, overrideReason, nextStatus, nextStatus === 'Executado' ? nowDateIso() : null, req.params.id);
    if (nextStatus === 'Executado') {
...
app.get('/cohorts/:id/suggestions/:moduleId', (req, res) => {
  ...
  `).get(cohortId, moduleId);
    ...
    order by
      ...
      c.name asc
  `).all(installationModuleId, moduleId, installationModuleId, installationModuleId, installationModuleId, moduleId, installationModuleId, installationModuleId, `Falta ${installationLabel}`, moduleId, moduleId, cohortId, moduleId);
...
app.delete('/companies/:id', (req, res) => {
    const exists = db.prepare('select id from company where id = ?').get(req.params.id);
    if (!exists) {
...
app.patch('/companies/:id/modules/:moduleId', (req, res) => {
    ...
    const module = db.prepare('select id from module_template where id = ?').get(req.params.moduleId);
    if (!module) {
...
app.post('/license-programs', (req, res) => {
    ...
    const payload = parsed.data;
    const existing = db.prepare('select id from license_program where lower(name) = lower(?)').get(payload.name.trim());
    if (existing) {
...
app.patch('/license-programs/:id', (req, res) => {
    ...
    const exists = db.prepare('select id from license_program where id = ?').get(req.params.id);
    if (!exists) {
    ...
    if (typeof parsed.data.name !== 'undefined') {
        const duplicate = db.prepare('select id from license_program where lower(name) = lower(?) and id <> ?').get(parsed.data.name.trim(), req.params.id);
        if (duplicate) {
...
app.delete('/license-programs/:id', (req, res) => {
    ...
    const usage = db.prepare('select count(*) as count from company_license where program_id = ?').get(req.params.id);
    if (usage.count > 0) {
...
app.post('/licenses', (req, res) => {
    ...
    const program = db.prepare('select id, name from license_program where id = ?').get(payload.program_id);
    if (!program) {
        return res.status(404).json({ message: 'Programa não encontrado' });
    }
    const nowIso = nowDateIso();
    ...
    try {
      ...
      values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, null, ?, ?)
    `).run(id, payload.company_id, program.name, payload.program_id, payload.user_name.trim(), payload.module_list.trim(), payload.license_identifier.trim(), payload.renewal_cycle, payload.expires_at, payload.notes ?? null, nowIso, nowIso);
        return res.status(201).json({ id });
    }
    catch (error) {
...
app.patch('/licenses/:id', (req, res) => {
    ...
    const exists = db.prepare('select id from company_license where id = ?').get(req.params.id);
    if (!exists) {
        return res.status(404).json({ message: 'Licença não encontrada' });
    }
    if (parsed.data.company_id) {
        const company = db.prepare('select id from company where id = ?').get(parsed.data.company_id);
        if (!company) {
            return res.status(404).json({ message: 'Cliente não encontrado' });
        }
    }
    ...
    if (typeof parsed.data.program_id !== 'undefined') {
        const program = db.prepare('select id, name from license_program where id = ?').get(parsed.data.program_id);
        if (!program) {
            return res.status(404).json({ message: 'Programa não encontrado' });
        }
...
app.get('/technicians', (_req, res) => {
  ...
  `).all();
    ...
    res.json(rows.map((t) => ({ ...t, skills: skillsByTech.get(t.id) ?? [] })));
});
...
app.patch('/technicians/:id', (req, res) => {
    ...
    const exists = db.prepare('select id from technician where id = ?').get(req.params.id);
    if (!exists) {
        return res.status(404).json({ message: 'Técnico não encontrado' });
    }
    const payload = parsed.data;
...
app.get('/technicians/:id/calendar', (req, res) => {
    ...
    where c.technician_id = ?
      ...
      and (? = '' or date(c.start_date) <= date(?))
    order by date(c.start_date) asc
  `).all(req.params.id, dateFrom, dateFrom, dateTo, dateTo);
    const blocks = db.prepare(`
...
app.post('/admin/modules', (req, res) => {
    ...
    try {
      ...
      values (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, parsed.data.code.toUpperCase(), parsed.data.category, parsed.data.name, parsed.data.description ?? null, parsed.data.duration_days, parsed.data.profile ?? null, parsed.data.is_mandatory);
...
app.delete('/admin/modules/:id', (req, res) => {
    const module = db.prepare('select id, code, name from module_template where id = ?').get(req.params.id);
    if (!module) {
        return res.status(404).json({ message: 'Modulo nao encontrado' });
    }
    const installationCode = getInstallationModuleCode();
    ...
    const blockUsage = db.prepare('select count(*) as count from cohort_module_block where module_id = ?').get(module.id);
    ...
    const allocationUsage = db.prepare('select count(*) as count from cohort_allocation where module_id = ?').get(module.id);
...
app.post('/admin/bootstrap-current-data', (req, res) => {
    ...
    `);
        ...
        modules.forEach((module) => {
            upsertModule.run(`mod-${slugify(module.code)}`, module.code.trim().toUpperCase(), module.category ?? 'Geral', module.name.trim(), module.duration_days ?? 1, module.profile ?? null, module.is_mandatory ?? 0);
            summary.modules_upserted += 1;
        });
        const companies = db.prepare('select id from company').all();
        const modulesRows = db.prepare('select id from module_template').all();
...
app.post('/admin/bootstrap-real-scenario', (_req, res) => {
    ...
    `);
        ...
        modules.forEach((module) => {
            insertModule.run(`mod-${slugify(module.code)}`, module.code, module.category, module.name, module.duration_days, module.is_mandatory);
    ...
    `);
        ...
        realCohorts.forEach((cohort) => {
            insertCohort.run(cohort.id, cohort.code, cohort.name, cohort.start_date, cohort.technician_id, cohort.status, cohort.capacity_companies, 'Cenário real para validação');
            cohort.blocks.forEach((block) => {
                insertBlock.run(uuid('blk'), cohort.id, `mod-${slugify(block.module_code)}`, block.order_in_cohort, block.start_day_offset, block.duration_days);
            });
            cohort.allocations.forEach((allocation) => {
                insertAllocation.run(uuid('all'), cohort.id, `comp-${slugify(allocation.company_name)}`, `mod-${slugify(allocation.module_code)}`, allocation.entry_day, allocation.status);
