...
export function initDb() {
  ...
  `);
    ...
    ensureColumn('cohort_allocation', 'override_installation_prereq', 'override_installation_prereq integer not null default 0');
    ...
    const licenseProgramCount = db.prepare('select count(*) as count from license_program').get();
...
function hasSeed() {
    const row = db.prepare('select count(*) as count from module_template').get();
    return row.count > 0;
}
export function seedDb() {
    ...
    const insertModule = db.prepare('insert into module_template (id, code, category, name, duration_days, profile, is_mandatory) values (?, ?, ?, ?, ?, ?, ?)');
    ...
    const progress = db.prepare('insert into company_module_progress (id, company_id, module_id, status, completed_at) values (?, ?, ?, ?, ?)');
    ...
    const activation = db.prepare('insert or ignore into company_module_activation (company_id, module_id, is_enabled) values (?, ?, 1)');
    companies.forEach((company) => {
        modules.forEach((module) => {
            activation.run(company[0], module[0]);
        });
    });
    const cohorts = db.prepare('insert into cohort (id, code, name, start_date, technician_id, status, capacity_companies, notes) values (?, ?, ?, ?, ?, ?, ?, ?)');
    ...
    const blocks = db.prepare('insert into cohort_module_block (id, cohort_id, module_id, order_in_cohort, start_day_offset, duration_days) values (?, ?, ?, ?, ?, ?)');
    ...
    const allocations = db.prepare('insert into cohort_allocation (id, cohort_id, company_id, module_id, entry_day, status, notes) values (?, ?, ?, ?, ?, ?, ?)');
    ...
    const prereq = db.prepare('insert or ignore into module_prerequisite (module_id, prerequisite_module_id) values (?, ?)');
    modules
