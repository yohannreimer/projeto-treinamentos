...
export type ImportSummary = {
  ...
  technicians: number;
  cohorts: number;
  cohort_blocks: number;
  allocations: number;
  optionals: number;
  company_progress_updates: number;
  optional_progress_updates: number;
};
...
function pick(row: AnyRow, aliases: string[]): unknown {
  const entries = Object.entries(row);
  for (const alias of aliases) {
    const normalizedAlias = normalizeKey(alias);
    for (const [key, value] of entries) {
      if (normalizeKey(key) === normalizedAlias) return value;
    }
  }
  return undefined;
}
...
function normalizeCompanyStatus(value: unknown): string {
  const text = normalizeText(toStringValue(value));
  if (text.includes('inativ')) return 'Inativo';
  return 'Ativo';
}

function normalizeModuleStatus(value: unknown): string {
  const text = normalizeText(toStringValue(value));
  if (!text) return 'Nao_iniciado';
  if (text.includes('conclu')) return 'Concluido';
  if (text.includes('execu')) return 'Em_execucao';
  if (text.includes('planej')) return 'Planejado';
  if (text.includes('nao')) return 'Nao_iniciado';
  return 'Nao_iniciado';
}

function normalizeOptionalStatus(value: unknown): string {
  const text = normalizeText(toStringValue(value));
...
function normalizeAllocationStatus(value: unknown): string {
  const text = normalizeText(toStringValue(value));
  if (text.includes('execu')) return 'Executado';
  if (text.includes('confirm')) return 'Confirmado';
  if (text.includes('cancel')) return 'Cancelado';
  return 'Previsto';
}

function normalizeCohortStatus(value: unknown): string {
  const text = normalizeText(toStringValue(value));
  ...
  if (text.includes('quorum')) return 'Aguardando_quorum';
  return 'Planejada';
}

function isMandatory(value: unknown): number {
  const text = normalizeText(toStringValue(value));
  if (text.includes('sim') || text.includes('obrig')) return 1;
  return 0;
}
...
function ensureCompany(name: string, companyIdsByName: Map<string, string>): string {
  ...
  db.prepare('insert into company (id, name, status, notes, priority) values (?, ?, ?, ?, ?)').run(
    id,
    name,
    'Ativo',
    'Criado automaticamente no import',
    0
  );
...
function ensureTechnician(name: string, techIdsByName: Map<string, string>): string {
  const key = normalizeText(name);
  const existing = techIdsByName.get(key);
  if (existing) return existing;

  const id = `tech-${slug(name)}-${Math.random().toString(36).slice(2, 7)}`;
  db.prepare('insert into technician (id, name, availability_notes) values (?, ?, ?)').run(
    id,
    name,
    null
  );
...
export function importWorkbook(filePath: string, options: ImportOptions = {}): ImportSummary {
  ...
  const tx = db.transaction(() => {
    ...
    const moduleIdsByCode = new Map<string, string>();
    const companyIdsByName = new Map<string, string>();
    const techIdsByName = new Map<string, string>();
    const cohortIdsByCode = new Map<string, string>();
    const optionalIdsByCode = new Map<string, string>();
    ...
    for (const row of progressRows) {
      ...
      const moduleCode = toStringValue(pick(row, ['Codigo_Modulo', 'Código_Modulo'])).toUpperCase();
      if (!companyName || !moduleCode) continue;
    ...
    for (const row of optionalProgressRows) {
      ...
      if (!companyName || !optionalCode) continue;

      const companyId = ensureCompany(companyName, companyIdsByName);
      const optionalId = optionalIdsByCode.get(optionalCode);
      if (!optionalId) continue;
    ...
    for (const row of allocationRows) {
      ...
      const companyName = toStringValue(pick(row, ['Empresa']));
      const moduleCode = toStringValue(pick(row, ['Codigo_Modulo', 'Código_Modulo'])).toUpperCase();
      if (!cohortCode || !companyName || !moduleCode) continue;

      const cohortId = cohortIdsByCode.get(cohortCode);
      const companyId = ensureCompany(companyName, companyIdsByName);
      const moduleId = moduleIdsByCode.get(moduleCode);
      if (!cohortId || !companyId || !moduleId) continue;
      ...
      upsertAllocation.run(
        `all-${Math.random().toString(36).slice(2, 12)}`,
        cohortId,
        companyId,
        moduleId,
        entryDay,
